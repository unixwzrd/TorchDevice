...
from tests.common.log_diff import setup_log_capture, teardown_log_capture
...
class TestCPUOverride(PrefixedTestCase):
    ...
    def test_cpu_override_device_creation(self):
        ...
        if hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
            ...
            # Even with CPU override, non-CPU devices should still redirect to CPU
            self.assertEqual(device3.type, 'cpu')
        
        # Check our log output
        log_output = self.log_capture.log_stream.getvalue()
        self.assertIn("Explicitly overriding default device to CPU", log_output)
        self.assertIn("Respecting explicit CPU device request", log_output)
