...
class TorchDevice:
    ...
    def mock_cuda_stream(cls, stream=None):
        ...
        if cls.get_default_device() == 'cuda':
            ...
            # If MPS is available, try to use the real MPS Stream class
            try:
                import torch.mps
                if hasattr(torch.mps, 'Stream'):
                    log_info("Creating MPS stream.", "torch.cuda.stream -> torch.mps.Stream", device_type='mps')
    ...
    def mock_cuda_event(cls, enable_timing=False, blocking=False, interprocess=False):
        """Mock for torch.cuda.Event class."""
        if cls.get_default_device() == 'cuda':
            log_info("Creating CUDA event (mocked).", "torch.cuda.Event", device_type='cuda')
            # Return a simple object that mimics a CUDA event
            class MockEvent:
                def __init__(self):
                ...
                def record(self, stream=None):
                    log_info("Event record called.", "torch.cuda.Event.record", device_type='cuda')
                    self._recorded = True
                    return self
                
                def wait(self, stream=None):
                    log_info("Event wait called.", "torch.cuda.Event.wait", device_type='cuda')
                    return self
                
                def query(self):
                    log_info("Event query called.", "torch.cuda.Event.query", device_type='cuda')
                    return True
                
                def synchronize(self):
                    log_info("Event synchronize called.", "torch.cuda.Event.synchronize", device_type='cuda')
                    return self
                
                def elapsed_time(self, end_event):
                    log_info("Event elapsed_time called.", "torch.cuda.Event.elapsed_time", device_type='cuda')
                    return 0.0
                
                def __eq__(self, other):
                    return isinstance(other, MockEvent) and self._recorded == other._recorded
            
            return MockEvent()
        elif cls.get_default_device() == 'mps':
            # If MPS is available, use the real MPS Event class
            try:
                import torch.mps.event
                log_info("Creating MPS event.", "torch.cuda.Event -> torch.mps.event.Event", device_type='mps')
                return torch.mps.event.Event(enable_timing=enable_timing)
            except (ImportError, AttributeError):
                log_warning("MPS Event not available, using mock implementation.", "torch.cuda.Event", device_type='mps')
                # Fall back to mock implementation if MPS Event is not available
                class MockMPSEvent:
                    def __init__(self):
                        self.enable_timing = enable_timing
                        self._recorded = False
            ...
            log_warning("No GPU available to create event.", "torch.cuda.Event", device_type='cpu')
            return None
