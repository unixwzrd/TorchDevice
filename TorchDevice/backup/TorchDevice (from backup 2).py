...
class TorchDevice:
    ...
    def apply_patches(cls):
        ...
        log_info("Patched torch.serialization._get_restore_location for TTS compatibility", device_type=cls.get_default_device())
        ...
        try:
            ...
            def patched_numpy_to_torch(np_array, dtype, device=None):
                ...
                if device is not None:
                    ...
                    if device == 'cuda' and hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
                        log_info("Redirecting tensor from 'cuda' to 'mps' in TTS synthesis", device_type='mps')
                        device = 'mps'
            ...
            log_info("Patched TTS.tts.utils.synthesis.numpy_to_torch for device compatibility", device_type=cls.get_default_device())
        except ImportError:
            # TTS is not installed, so no need to patch
            log_info("TTS not installed, skipping TTS-specific patches", device_type=cls.get_default_device())
