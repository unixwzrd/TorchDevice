...
@classmethod
def get_dummy_cuda_event(cls):
    log_warning("CUDA Event is not supported on this device.", "torch.cuda.Event")
    try:
        from torch.cuda import _EventBase
    except ImportError:
        _EventBase = object
    class DummyCudaEvent(_EventBase):
        def __init__(self, *args, **kwargs):
            # You can add any initialization behavior if needed.
            pass
    ...
    def apply_patches(cls):
        ...
        # Handles teh Event class
        torch.cuda.Event = cls.get_dummy_cuda_event()
