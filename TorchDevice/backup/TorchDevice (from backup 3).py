    ...
    def mock_cuda_event(cls, enable_timing=False, blocking=False, interprocess=False):
        ...
        try:
            ...
            if hasattr(torch.mps, 'event') and hasattr(torch.mps.event, 'Event'):
                ...
                # Add a mock elapsed_time method that always returns a fixed value
                # This prevents hanging when calling elapsed_time
                def elapsed_time_wrapper(end_event):
                    log_info("Event elapsed_time called, returning mock value for MPS compatibility.", 
                             "torch.mps.event.Event.elapsed_time", device_type='mps')
                    # Return a small mock value in milliseconds
                    return 0.5
                
                # Replace methods
                mps_event.record = record_wrapper
                
                # Always replace elapsed_time to avoid hanging
                mps_event.elapsed_time = elapsed_time_wrapper
